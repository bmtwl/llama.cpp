...
class ModelBase:
    ...
    def prepare_tensors(self):
        ...
        # Collect all tensors into a dictionary to handle FP8 scale tensors
        all_tensors = {}
        for name, data_torch in chain(self.generate_extra_tensors(), self.get_tensors()):
            all_tensors[name] = data_torch

        for name, data_torch in all_tensors.items():
            # we don't need these
            if name.endswith((".attention.masked_bias", ".attention.bias", ".rotary_emb.inv_freq", "_scale_inv")):
                continue

            # Check if this is an FP8 tensor and has a scale tensor
            if data_torch.dtype == torch.float8_e4m3fn:
                scale_name = name + '_scale_inv'
                if scale_name in all_tensors:
                    # tile the scales to match the data shape
                    scale = all_tensors[scale_name].to(torch.float32)
                    # Dynamically compute repeat factors for each dimension
                    for dim in range(scale.ndim):
                        data_dim = data_torch.shape[dim]
                        scale_dim = scale.shape[dim]
                        repeat_factor = (data_dim + scale_dim - 1) // scale_dim
                        scale = scale.repeat_interleave(repeat_factor, dim=dim)
                        # Slice to match data_dim if necessary
                        if scale.shape[dim] > data_dim:
                            scale = scale.narrow(dim, 0, data_dim) 
                    data_torch = data_torch.to(torch.float32) * scale
                    self.tensor_names.remove(scale_name)
            ...
            if data_torch.dtype not in (torch.float16, torch.float32):
                ...
                all_tensors[name] = data_torch  # Update the tensor in the dictionary
